...
const ALLOWED_CONTRACT = `eosio.token`
const checkAction = (action: any): void => {
    switch (action.account) {
        case ALLOWED_CONTRACT: {
            // any internal action except noop is fine
            // we don't want someone to DDOS by sending only noop actions
            if (action.name === `noop`) {
                throw new Error(`Don't include duplicate noop actions.`)
            }
            return;
        }
        default: {
...
router.post('/sign', async (req: Request, res: Response) => {
    try {
        ...
        // insert cpu payer's noop action as first action to trigger ONLY_BILL_FIRST_AUTHORIZER
		tx.actions.unshift({
			account: "res.pink",
			name: "noop",
			authorization: [
